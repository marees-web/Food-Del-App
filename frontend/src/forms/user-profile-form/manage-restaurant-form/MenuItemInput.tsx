    ...
    <div className="flex flex-row items-end gap-2">
        <FormField control={control} name={`menuItems.${index}.name`} render={({field})=>(
            <FormItem>
                ...
                <FormControl>
                    <Input {...field} placeholder="Tomato pasta" className="bg-white"/>
                </FormControl>
            </FormItem>
        )}
        />
        ...
        />
        <Button type="button" onClick={removeMenuItem} className="bg-red-500 max-h-fit hover:bg-red-600 ">Remove</Button>
    </div>
  )
}
